...
const defaultData = {
  ...
  bookings: [
    ...
    { id: 6, staffName: '三歲', time: '20:00', plan: '40', price: '', customer: '', note: '' },
    { id: 7, staffName: '妍妍', time: '19:20', plan: '60', price: '', customer: '', note: '' },
  ],
};
...
function getNowMinutesForBusinessDay() {
  const now = new Date();
...
export default function App() {
  ...
  const [staffForm, setStaffForm] = useState({ name: '', area: '', planText: '方案20/40/60/2+1/3+1/5+3', priceText: '底1.5/2.0/2.3' });
  ...
  const scheduleText = useMemo(() => {
    ...
    sortedStaff.forEach((person, index) => {
      ...
      // 先顯示現在狀態
      lines.push(getCurrentLabel(personBookings));

      // 再顯示未來預約 + 該筆結束後可約時間
      const futureBookings = getFutureBookings(personBookings);
      futureBookings.forEach((item) => {
        const endTime = addPlanMinutes(item.time, item.plan);
        lines.push(`${item.time.replace(':', '.')}／${item.plan}`);
        lines.push(`👉${minutesToDisplay(endTime)}可約💦`);
      });
  ...
  const quickAddBooking = () => {
    ...
    if (parts.length < 3) {
      setMessage('格式錯誤，請用：人員/時間/方案/價格/客戶名稱/備註');
      return;
    }
  ...
  const addStaff = () => {
    ...
    setStaffForm({ name: '', area: '', planText: '方案20/40/60/2+1/3+1/5+3', priceText: '底1.5/2.0/2.3' });
    setMessage('已新增人員');
  };
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              ...
              <div style={sectionStyle()}>
                ...
                <input
                  ...
                  placeholder="糖糖/18:10/2+1/2300/王哥/老客"
                />
